...
The default rooms task uses a coverage observation with several nearby unvisited checkpoints, not only one next checkpoint. Each checkpoint slot points to a reachable visible point sampled along the checkpoint, which keeps the policy from aiming at endpoints close to walls or already visited gates. PPO is also run with a small entropy coefficient by default, and the environment gives penalties for hovering, repeated wall contact, and not approaching a visible unvisited checkpoint. Hovering in explored space gives `-0.004` per step, and failing to approach a visible unvisited checkpoint gives `-0.002` per step. Checkpoint collection requires crossing the checkpoint line; driving nearby without crossing does not give reward. The rooms task also uses slower, more damped robot dynamics than the original racing-track task so the robot can turn into narrow gates. Wall contact penalties increase within each checkpoint-to-checkpoint segment but are bounded and reset after reaching a new checkpoint. These changes make the task behave more like exploration than the older lap-following example.
